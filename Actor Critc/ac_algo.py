...
class Actor(object):
    def __init__(self, sess, a_dim, n_features, action_bound, lr=0.001):
        ...
        ##squeeze 去掉值为1的维度[[0]] ----> 0 --Remove dimensions with a value of 1
        # self.mu, self.sigma = tf.squeeze(mu * 2), tf.squeeze(sigma + 0.1)
        self.mu, self.sigma = tf.squeeze(mu), tf.squeeze(sigma + 0.1)
        # tf.distributions.normal可以生成一个均值为loc，方差为scale的正态分布。--can generate a mean value of loc  --Normal distribution with variance scale.
        self.normal_dist = tf.distributions.Normal(self.mu, self.sigma)

        ##tf.clip_by_value:输入一个张量A，把A中的每一个元素的值都压缩在min和max之间。小于min的让它等于min，大于max的元素的值等于max。 --Input a tensor A, and compress the value of each element in A between min and max. Let it be equal to min if it is less than min, and the value of the element greater than max is equal to max
        # sample输出value值，在正态分布中概率的log值，作为loss值  --Output the value value, the log value of the probability in the normal distribution, as the loss value
