...
class UAVEnv(object):
    ...
    task_list = np.random.randint(
        2097153, 2621440, M)  # 随机计算任务2~2.5Mbits -> 80
    # ue位置转移概率
    ...
    def __init__(self):
        ...
        self.start_state = np.append(
            self.start_state, np.ravel(self.loc_ue_list))
    ...
    def reset_env(self):
        ...
        self.loc_uav = [50, 50]
        self.loc_ue_list = np.random.randint(
            0, 101, size=[self.M, 2])  # 位置信息:x在0-100随机
        self.reset_step()
    ...
    def reset_step(self):
        ...
        # 随机计算任务1.5~2Mbits -> 1.5~2 2~2.5 2.5~3 3~3.5 3.5~4
    ...
    def step(self, action):  # 0: 选择服务的ue编号 ; 1: 方向theta; 2: 距离d; 3: offloading ratio
        ...
        # 服务器计算耗能
        t_server = offloading_ratio * task_size / \
            (self.f_uav / self.s)  # 在UAV边缘服务器上计算时延
        ...
        # 最后一步计算任务和ue的计算任务不匹配
            ...
            # 如果超出边界，则飞行距离dist置零
            reset_dist = True
            delay = self.com_delay(
                self.loc_ue_list[ue_id], self.loc_uav, offloading_ratio, task_size, block_flag)  # 计算delay
            reward = -delay
            # 更新下一时刻状态
            self.e_battery_uav = self.e_battery_uav - e_server  # uav 剩余电量
            self.reset2(
                delay, self.loc_uav[0], self.loc_uav[1], offloading_ratio, task_size, ue_id)
        elif self.e_battery_uav < e_fly or self.e_battery_uav - e_fly < e_server:  # uav电量不能支持计算
            delay = self.com_delay(self.loc_ue_list[ue_id], np.array([loc_uav_after_fly_x, loc_uav_after_fly_y]),
                                   0, task_size, block_flag)  # 计算delay
            reward = -delay
            self.reset2(delay, loc_uav_after_fly_x,
                        loc_uav_after_fly_y, 0, task_size, ue_id)
    ...
    def reset2(self, delay, x, y, offloading_ratio, task_size, ue_id):
        ...
        for i in range(self.M):  # ue随机移动后的位置
            ...
            self.loc_ue_list[i][0] = self.loc_ue_list[i][0] + \
                math.cos(theta_ue) * dis_ue
            self.loc_ue_list[i][1] = self.loc_ue_list[i][1] + \
                math.sin(theta_ue) * dis_ue
            self.loc_ue_list[i] = np.clip(
                self.loc_ue_list[i], 0, self.ground_width)
    ...
    def com_delay(self, loc_ue, loc_uav, offloading_ratio, task_size, block_flag):
        ...
        trans_rate = self.B * \
            math.log2(1 + self.p_uplink * g_uav_ue / p_noise)  # 上行链路传输速率bps
        t_tr = offloading_ratio * task_size / trans_rate  # 上传时延,1B=8bit
        t_edge_com = offloading_ratio * task_size / \
            (self.f_uav / self.s)  # 在UAV边缘服务器上计算时延
        t_local_com = (1 - offloading_ratio) * task_size / \
            (self.f_ue / self.s)  # 本地计算时延
